...
void benchmark(int num_thread, int tid)
{
	for (int i = 0; i < NUM_TEST / num_thread; ++i) {
		//	if (0 == i % 100000) cout << ".";
		switch (my_rand() % 3) {
		case 0: my_set.Add(my_rand() % RANGE, tid); break;
		case 1: my_set.Remove(my_rand() % RANGE, tid); break;
		case 2: my_set.Contains(my_rand() % RANGE, tid); break;
		default: cout << "ERROR!!!\n"; exit(-1);
		}
	}
}
